...
router.get('/', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    res.json({ success: true, data: { notifications: mappedNotifications, unreadCount, ...buildPaginationResponse(total || 0, page, limit) } });
  } catch (err) { next(err); }
});
...
router.patch('/:id/read', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    await markAsRead(req.user!.id, req.params.id as string);
    const unreadCount = await getUnreadCount(req.user!.id);
...
// POST /api/notifications/listing-created
router.post('/listing-created', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { listing } = req.body;
    if (!listing) throw createApiError(400, 'Listing data is required');

    // In a real app, this would query matching buyers based on district and crop preferences.
    // For now, we mock notifying interested buyers.
