...
const TABS = [
  { id: 'all', label: 'All' },
  { id: 'pending_farmer', label: 'Pending Farmer' },
...
export default function BuyerOrdersPage() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      // We will calculate analytics directly from the orders list for now
      // A dedicated /analytics route would be better for pagination
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        const pending = fetchedOrders.filter((o: any) => ['pending_farmer', 'confirmed'].includes(o.status)).length;
        const completed = fetchedOrders.filter((o: any) => o.status === 'completed').length;
        const spent = fetchedOrders.reduce((sum: number, o: any) => sum + o.totalAmount, 0);
  ...
  const handlePay = async (id: string) => {
    ...
    // In reality, this would redirect to a checkout page or open a modal
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-24">
      ...
      {/* Analytics Summary */}
      <OrdersAnalytics data={analytics} loading={analyticsLoading} role="buyer" />
      ...
      <div className="min-h-[400px]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center bg-white dark:bg-gray-900 rounded-xl border">
            ...
            <p className="text-gray-500 mt-2">Try adjusting your filters or place a new order from the marketplace.</p>
            <Button variant="outline" className="mt-6" onClick={() => router.push('/buyer/marketplace')}>
              Go to Marketplace
            </Button>
          </div>
        ) : (
